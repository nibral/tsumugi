            ...
            $('.timetb-ag > tbody > tr').each((rowIndex, row) => {
                ...
                $(row).children('td').each((colIndex, element) => {
                    ...
                    */
                    ...
                    if (time && title && !title.match(/放送休止/)) {
                        // パーソナリティが設定されていない番組もある
                        if (!personality) {
                            personality = '';
                        }

                        timetable[startDayNo + 1].push({
                            time: time,
                            title: title,
        ...
        */
        ...
        while (!nowProgram) {
            ...
            for (let i = 0; i < timetable[startDayNo].length; i++) {
                if (timetable[startDayNo][i].time === startTimeStr) {
                    nowProgram = timetable[startDayNo][i];
                    break;
                }
            }
        ...
        for (let i = 0; i < timetable[nextStartDayNo].length; i++) {
            if (timetable[nextStartDayNo][i].time === nextStartTimeStr) {
                nextProgram = timetable[nextStartDayNo][i];
                break;
            }
        }
