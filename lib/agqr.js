...
const searchProgramFromTimetable = (time) => {
    ...
    while (!nowProgram) {
        // 曜日と開始時刻が一致する番組を検索
        const startTimeStr = getTimeString(time);
        const startDayNo = (time.getDay() === 0) ? 7 : time.getDay();
...
/*
    番組表から現在放送中の番組の情報を取得
*/
module.exports.getNowProgramFromTimetable = () => {
    return new Promise((resolve, reject) => {
        ...
        // 放送中の番組を探す
        const nowProgram = searchProgramFromTimetable(startTime);
        ...
        const nextStartTime =
            new Date(startTime.getTime() + nowProgram.length * 60 * 1000);
        const nextStartTimeStr = getTimeString(nextStartTime);
        const nextStartDayNo =
            (nextStartTime.getDay() === 0) ? 7 : nextStartTime.getDay();
